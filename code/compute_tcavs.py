...
def compute_word_tcav(concept_cavs, random_cavs,
    ...
    for lx in list(["1"]):
        ...
        for concept, cavs_of_runs in layer_cavs.items():
            ...
            logging.info(f"Running for Concept - {concept}")
            tcavs_per_run = []
            ...
            for run in range(num_of_runs):
                ...
                for cav_key, cav in cavs_of_runs[run].items():
                    if if_rand:
                        random_cav = random_layer_cavs_per_concept[run][cav_key]
                    for jx, sent in enumerate(sentences):
                        ...
                        if word in words:
                            ...
                            logging.info(f"[INFO] The actual label was {gold_label} and the tag of prediction is {tag_of_prediction}.")
                            logging.info(f"[INFO] This was tested on concept {concept}.")
            ...
            logging.info(f"[INFO] Layer - {lx} TCAVs - {tcavs_per_run}, Masked Concept - {gold_label} Tested Concept - {concept}.")
            logging.info(f"[INFO] Random TCAVs - {random_tcavs_per_run}.")
            ...
            logging.info(f"[INFO] For Concept {concept}, the p-value is {p} at layer {lx}.")
...
def modify_extractions_with_added_masked_word(model_name, masked_sents,
    ...
    for concept_masked, sents in masked_sents.items():
        if concept_masked in ["NN", "CD", "VB", "MD", "JJ", "CC"]:
            logging.info(f"[INFO] Concept Masked - {concept_masked}")
...
def run_for_chosen_word_write_to_pickle(sentences, concept_cavs, random_cavs,
    ...
    for concept_masked, sents in sentences.items(): # these are concept_wise masked sentences.
        if concept_masked in ["NN", "CD", "VB", "MD", "JJ", "CC"]:
            logging.info(f"[INFO] Masked Concept - {concept_masked}")
            print(f"[INFO] Masked Concept - {concept_masked}")
...
def main():
    ...
    print(f"[INFO] Computing TCAVs for experiment {name}.")
    logging.info(f"Computing TCAVs for experiment {name}.")
